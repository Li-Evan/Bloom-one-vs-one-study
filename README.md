...
1. **读取日志基线**：读取 `knowledge/learning/learning-log.jsonl`，获取最后一条记录的 `date` 和 `courses`（各课题最后读到哪篇文档）
2. **扫描所有课题文件夹**：列出 `knowledge/learning/` 下所有子文件夹（排除隐藏目录和 `.templates`），对每个课题：
   - 列出所有 `XX.md` 文档（排除 `syllabus.md`、`summary.md`）
   - 与日志基线对比，找出**新增的文档**（即上次记录之后完成的）
   - 读取这些新文档，提炼 3-5 个核心概念/知识点
3. **生成日志条目**：构造以下 JSON 对象（单行），**追加**到 `learning-log.jsonl`：
...
1. **先读 `learning-log.jsonl`**（位于 `knowledge/learning/learning-log.jsonl`）—— 它记录了所有课题的最新进度、已完成文档、核心概念摘要
...
- 每次对话开始时，**先读 `learning-log.jsonl`** 了解整体学习状态；若需处理某具体课题，再读该课题的 `syllabus.md` 确认边界
